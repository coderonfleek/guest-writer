...
In the command above, `npx` invokes `degit` to clone the standard template in your current working directory. `svelte-task-list` is the folder in which the project code will be cloned. Once this process is complete, go into the root of your project by running `cd svelte-task-list`.
...
The next task is to set up all the application needs to authenticate users. To begin, you will need to install Auth0's SDK for authenticating Single Page Applications, the [@auth0/auth0-spa-js](https://www.npmjs.com/package/@auth0/auth0-spa-js) package. Install this package by running the following command at the root of your project:
...
The above code consists of five (5) writable and one derived state properties:
...
- `popupOpen`: The sign-in process will be initiated using Auth0's popup authentication modal, this is to monitor the visible state of the pop up modal.
...
Ensure to replace `YOUR_AUTH0_DOMAIN` with your Auth0 domain signature and `YOUR_APP_CLIENT_ID` with the Client Id of the Auth0 application created at the beginning of this tutorial. Make sure you add this file to your `.gitignore` file so that you don't mistakenly push your details to a public repository.

With this file in place, you can now create the authentication service, create the file `authService.js` inside the `src` folder and place in it the following code:
...
The file above begins by importing the required objects from the `@auth0/auth0-spa-js` package, the application store (`store.js`) and the authentication configurations from `auth_config.js`. Three (3) authentication functions are then defined as follows:
...
You will create a task list item component that will display and control the behaviour of a task item in the application. Inside the `src` folder, create a new folder with the name `components`, and inside this folder, create the file `TaskItem.svelte` and input the following code:
...
For some basic styling, the [Bootstrap](getbootstrap.com/) library will be used. To apply Bootstrap, go into the `public/index.html` file and add the library (using a CDN link) in the `head` section under the `meta` tags as shown below:
...
Next, remove the reference to `global.css`, you will not be needing it and some of the styles in the file clashes with those you will define later on.
...
The script begins by importing all required objects (`onMount` from `svelte`, `isAuthenticated`, `user`, `user_tasks`, `tasks` from the store), services (the `authService` module) and components (`TaskList.svelte`).
...
This simply set's a margin of `10px` at the top of the application's template for the authenticated screen which will be defined next.

### Adding authentication to the Task List page

The next step is to create the application screen by defining the template. The screen will consist of a Bootstrap navigation bar that will display the application name and, based on the authentication state, will display the email of the name and email of the logged-in user and also conditionally display a `Log In/Log Out` link.
...
![Welcome Screen](images/2020-11-17-app-home.png "Welcome Screen - Task Manager")

Now click on any of the `Log In` button/link to sign-in to the application, this will pop up the Auth0 log-in screen as shown below:

![Login Pop Up](images/2020-11-17-login-to-app.png "Login Pop Up - Task Manager")

You can use any of the authentication options displayed but for this demo, use the Google log-in button and once the process is completed, you will be authenticated and the task list page will be displayed as shown below:
...
![App page, no items](images/2020-11-17-task-list-blank.png "App page, no items - Task Manager")
...
![App page with items](images/2020-11-17-task-list-ticked.png "App page with items - Task Manager")
...
Svelte is a very fast and easy-to-use frontend framework with a simple API that does not cheat you on any feature of modern frontend frameworks but rather does it efficiently. With Auth0, you have one less thing to worry about as you can now easily authenticate your Svelte apps as demonstrated in this tutorial.

If any part of your code is not working as expected, I advise that you go through the article once again to seee anything you may have missed.
